...
「イベント処理」とは、「ユーザーの行動」に合わせて、あらかじめプログラムしていたDOM操作を行うこと。
...
主に`addEventListener()`メソッドを使って作成する。
```
HTML要素.addEventListener('イベントの種類', () => {
  イベント処理
});
```
HTML要素の部分には、`getElementById()`メソドナをで取得したHTML要素を記述する。

イベントの種類
- click: クリックしたとき(マウスボタンを押して話したとき)
